...
------------------------------ BEGIN Operation.balance ------------------------------
      ...
      , ISNULL(TRY_CONVERT(NVARCHAR(150), JSON_VALUE(doc, N'$."RegisterName"')), '') [RegisterName]
      , TRY_CONVERT(DATE, JSON_VALUE(doc,N'$."BalanceDate"'),127) [BalanceDate]
      , ISNULL(TRY_CONVERT(NVARCHAR(150), JSON_VALUE(doc, N'$."Fields"')), '') [Fields]
      , ISNULL(TRY_CONVERT(NVARCHAR(150), JSON_VALUE(doc, N'$."GroupBy"')), '') [GroupBy]
      , ISNULL(TRY_CONVERT(MONEY, JSON_VALUE(doc,N'$."TopRows"')), 0) [TopRows]
      FROM dbo.[Documents]
      WHERE [operation] = '3E66A8C0-F80D-11EA-A4DB-81D4427614D7'
; 
GO
...
GO
GRANT SELECT ON dbo.[Operation.balance.v]TO PUBLIC; 
...
------------------------------ BEGIN Operation.load ------------------------------
      ...
      , ISNULL(TRY_CONVERT(MONEY, JSON_VALUE(doc,N'$."Amount"')), 0) [Amount]
      , TRY_CONVERT(UNIQUEIDENTIFIER, JSON_VALUE(doc, N'$."currency"')) [currency]
      , ISNULL(TRY_CONVERT(NVARCHAR(150), JSON_VALUE(doc, N'$."CSV"')), '') [CSV]
      , TRY_CONVERT(UNIQUEIDENTIFIER, JSON_VALUE(doc, N'$."UsersParent"')) [UsersParent]
      FROM dbo.[Documents]
      WHERE [operation] = '2F092D30-7577-11EA-A771-6F1F7E13F48A'
; 
GO
...
GO
GRANT SELECT ON dbo.[Operation.load.v]TO PUBLIC; 
...
------------------------------ BEGIN Operation.OperName1 ------------------------------
      ...
      FROM dbo.[Documents]
      WHERE [operation] = '54AA5310-102E-11EA-AA50-31ECFB22CD33'
; 
GO
...
GO
GRANT SELECT ON dbo.[Operation.OperName1.v]TO PUBLIC; 
...
------------------------------ BEGIN Operation.OperName2 ------------------------------
      ...
      FROM dbo.[Documents]
      WHERE [operation] = '8D128C20-3E20-11EA-A722-63A01E818155'
; 
GO
...
GO
GRANT SELECT ON dbo.[Operation.OperName2.v]TO PUBLIC; 
...
------------------------------ BEGIN Operation.respPerson ------------------------------
      ...
      , TRY_CONVERT(UNIQUEIDENTIFIER, JSON_VALUE(doc, N'$."currency"')) [currency]
      FROM dbo.[Documents]
      WHERE [operation] = '1D6E2830-5CA1-11EA-9517-37B915E1B97B'
; 
GO
...
GO
GRANT SELECT ON dbo.[Operation.respPerson.v]TO PUBLIC; 
